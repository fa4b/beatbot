...
# Start the Discord Bot with prefix commands and a background thread that polls the beat API.
...
class BotRunner
  ...
  def start_polling_thread
    @polling_thread = Thread.new do
      ...
      loop do
        begin
          update_presence(BeatFetcher.fetch)
        rescue StandardError => e
  ...
  def update_presence(beat)
    unless beat
      Rails.logger.warn('[BotRunner] Beat API returned nil - skipping update')
      return
    end
  ...
  def register_beat_command
    @bot.command(:beat, description: 'Current Swatch Internet Beat time (BMT).') do |event|
      ...
      if beat
        ...
        event.respond("Sorry, I couldn't fetch the current beat right now.")
      end
    end
  end
  ...
  def register_worldtime_command
    @bot.command(:worldtime, description: 'Get the current beat, local time, and UTC time in an embed message.') do |event|
      unless valid_event?(event)
        event.respond("Error: Unable to process command")
        next
      end
      ...
      if data
        ...
        embed.add_field(name: 'Internet Beat', value: 'API unavailable', inline: true)
      end
